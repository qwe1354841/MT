...
Act_GuiWang.ShowItem = "61024,61025"

-- 活动开始前回调
function Act_GuiWang.OnDeclare(id, seconds)
    
end

-- 活动开始回调
function Act_GuiWang.OnStart(id, is_normal)
   
end

-- 活动结束回调
function Act_GuiWang.OnStop(id)
    
end

-- 玩家点击参与活动回调
function Act_GuiWang.OnJoin(id, player)
		--local x = 562
	--local y = 134
	--local range = 0
...
-- 玩家客户端查询数据回调
function Act_GuiWang.OnQuery(id, player)
	local str = [[0:0:0:0:]]..Act_GuiWang.ShowItem..[[:1:]]..ActivitySystem.Act_GetClassify("Act_GuiWang")
