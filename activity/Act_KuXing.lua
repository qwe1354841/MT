--苦行任务活动脚本
Act_KuXing = {}
Act_KuXing.ShowItem = "61024,61025,20022"
-- 活动开始前回调
function Act_KuXing.OnDeclare(id, seconds)
    
end

-- 活动开始回调
function Act_KuXing.OnStart(id, is_normal)
   
end

-- 活动结束回调
function Act_KuXing.OnStop(id)
    
end

-- 玩家点击参与活动回调
function Act_KuXing.OnJoin(id, player)
	--local x = 562
	--local y = 134
	--local range = 0
...
-- 玩家客户端查询数据回调
function Act_KuXing.OnQuery(id, player)
	local str = ""..sQuestSystem:GetRingWeek(player,115)..":100:0:0:"..Act_KuXing.ShowItem..":1:"..ActivitySystem.Act_GetClassify("Act_KuXing")
