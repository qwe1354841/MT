--攻守之道任务活动脚本
Act_GongShou = {}
Act_GongShou.ShowItem = "61025"
-- 活动开始前回调
function Act_GongShou.OnDeclare(id, seconds)
    
end

-- 活动开始回调
function Act_GongShou.OnStart(id, is_normal)
   
end

-- 活动结束回调
function Act_GongShou.OnStop(id)
    
end

-- 玩家点击参与活动回调
function Act_GongShou.OnJoin(id, player)
	--local x = 562
	--local y = 134
	--local range = 0
...
-- 玩家客户端查询数据回调
function Act_GongShou.OnQuery(id, player)
	local str = ""..sQuestSystem:GetRingWeek(player,120)..":10:0:0:"..Act_GongShou.ShowItem..":1:"..ActivitySystem.Act_GetClassify("Act_GongShou")
